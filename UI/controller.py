...
class Controller:
    ...
    def iscrivi(self, e):
        ...
        if studente is None:
            self._view.create_alert("Matricola non presente nel database")
            return
        codice_corso = self._view.corso.value
    ...
    def populate_dd_corso(self):
        for corso in self._model.get_corsi():
            ...
            self._view.corso.options.append(ft.dropdown.Option(key=corso.codins, text=corso))
        self._view.update_page()

    def leggi_corso(self, e):
        self.corso_selezionato = self._view.corso.value
